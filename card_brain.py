...
class CardBrain:
  ...
  def __init__(self, shuffled_rank, new_dict):
    ...
    self.new_dict = new_dict  # Output dict after remembered words are removed from the stack.
    ...
    self.canvas.grid(column=0, row=1, columnspan=4, rowspan=8)
    ...
    for i in range(1, 6):
      setattr(CardBrain, f"btn_stack_{i}", Button(text=f"Cards {i * 20 - 19} - {i * 20}", command=self.front_card,
                                                  highlightbackground=BACKGROUND_COLOR, font=self.button_style,
                                                  borderwidth=0))
    ...
    info_button.grid(column=4, row=0, pady=20)
    self.button_right = Button(text="I know this!", command=self.click_right, bd=0, highlightthickness=0,
                          highlightbackground=BACKGROUND_COLOR, font=self.button_style, padx=20, pady=5, borderwidth=0)
    self.button_wrong = Button(text="Need to review", command=self.click_right, bd=0, highlightthickness=0,
                          highlightbackground=BACKGROUND_COLOR, font=self.button_style, padx=10, pady=5, borderwidth=0)

    self.btn_stack_1.grid(column=0, row=5)
    self.btn_stack_2.grid(column=1, row=5)
    self.btn_stack_3.grid(column=2, row=5)
    self.btn_stack_4.grid(column=0, row=6)
    self.btn_stack_5.grid(column=1, row=6)

    #TODO: need to add a btn for home page.
  ...
  def home_page(self):
    pass
    # TODO: add a function to hide other pages.

  def hide_home_page(self):
    self.canvas.itemconfig(self.welcome_text, state="hidden")
    for i in range(1, 6):
      getattr(CardBrain, f"btn_stack_{i}").grid_forget()

  # showing cards ___________________________________________________________________
  def front_card(self):
    # TODO: FLAG pass user choice (stack) to go_thru_stack (change names) to generate shuffle and something.
    ...
    self.button_wrong.grid(column=0, row=9)
    self.button_right.grid(column=3, row=9)
